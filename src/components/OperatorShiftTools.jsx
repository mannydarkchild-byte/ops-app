...
import { deleteOwnShift, updateOwnShift } from "../services/workflows.js";
...
export function OperatorShiftTools({ shift, user, supervisors = [], onDone }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const updated = await updateOwnShift(user, shift, {
    ...
    <div className="mt-3 space-y-2">
      {!open ? (
        ...
        <div className="bg-[#0A0A0A] border border-[#2A2A2A] rounded-xl p-4 text-left">
          <p className="font-logo text-[#F5C518] mb-3">Edit this shift</p>
